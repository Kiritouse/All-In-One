...
rbegin() //返回指向容器最后一个元素的反向迭代器it,it++是从最后一个遍历到第一个 
rend() //返回容器的第一个元素的前一个的反向迭代器 
front()  
back()  
erase()   //vec.erase(vec.begin() + 2);  删除第三个元素（值为3）
// vec.erase(vec.begin() + 1, vec.begin() + 4);删除一段元素
...
    if (it != myMap.end()) {
        std::cout << "Key " << keyToFind << " found with value: " << it->second << std::endl;
    } else {
...
```c++
set.insert(num);
set.erase(3);//删除值为多少的元素
//查找元素
if (mySet.find(2) != mySet.end()) {
    std::cout << "Found 2" << std::endl;
}
set.count(key)这个也可以用来查找元素
```
### unordered_set
和set差不多,但是里面的元素是无序的
#### 常用操作
